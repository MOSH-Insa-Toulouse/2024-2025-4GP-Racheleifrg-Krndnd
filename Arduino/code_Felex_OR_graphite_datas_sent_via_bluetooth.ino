...
/select one of the four potentiometre values proposed) a rotary encoder is used.
 Its switch allows te user to confirm their selection.
At the begining of this code we can chose wether we want to send via bluetooth (by an HC-05 module)
the flex sensor's data or the graphite sensor's data and they are received by an application 
...
//choice between plotting Rflex or Rgraphite on the app
bool haschosen=false;
char inputchoice;//choice we make
byte *Rchosen;//pointer pointing to the right value to send over bluetooth
int choice=0;
...
void loop(){
  if (haschosen==false){ // we chose wich resitance we want to plot
    Serial.println(F("Choose one of the following option: 1) Draw Rgraphene, 2) Draw Rflex"));
    Serial.println(F("If you want to change the answer run the programm again   "));
    delay(2000);
    if(Serial.available()>0){
      
      inputchoice=Serial.read();
      
      if (inputchoice=='1'){
        Serial.println(F("You chose to draw Rgraphen"));
        choice=1;
        
        haschosen=true;
      }else if (inputchoice=='2'){
        Serial.println(F("You chose to draw Rflex"));
        choice=2;
        haschosen=true;
        
      }else{
        Serial.println(F("You chose a wrong answer,try again"));
      }
    }
  } else {//we already made a choice, we can make measures and use the oled/rotary encoder
    
    flex_measure();
    graphen_mesures();
    updateEncodeur();
    Rchosen=(choice==1)?&Vgraphsensor_bluetooth:&ADC_flex_bluetooth;//pointer pointing on the resistance we chose
    send_bluetooth( *Rchosen);
      #ifdef DEB_POT
    
    Serial.print(F("ADC_chosen ")); 
    Serial.print(*Rchosen); 
